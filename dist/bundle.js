(() => {
  ...
  var Player = class extends Entity {
    constructor(x, y) {
      ...
      Object.keys(this.images).forEach((k) => {
        ...
        this.images[k].onload = () => {
          if (++loaded === 6) console.log("\u2705 Astronaut assets loaded");
        };
      });
      this.config = {
        bodyY: 115,
    ...
    isAtPortal() {
      ...
      if (this.mode === "maze") {
        return this.mazeRow === interior.exitRow && (this.mazeCol === interior.exitColLeft || this.mazeCol === interior.exitColRight);
    ...
    drawAstronaut(ctx, drawPos) {
      ...
      this.drawPart(
        ctx,
        this.images.leftboot,
        cx + this.config.leftBootX * s,
        cy + this.config.leftBootY * s,
        this.config.leftBootJointX,
        this.config.leftBootJointY,
        leftBootAngle
      );
      this.drawPart(
        ctx,
        ...
        this.config.leftArmJointX,
        this.config.leftArmJointY,
        0
      );
      ...
      this.drawPart(
        ctx,
        this.images.rightboot,
        cx + this.config.rightBootX * s,
        cy + this.config.rightBootY * s,
        this.config.rightBootJointX,
        this.config.rightBootJointY,
        rightBootAngle
      );
      this.drawPart(
        ctx,
        this.images.rightarm,
        cx + this.config.rightArmX * s,
        cy + this.config.rightArmY * s,
        this.config.rightArmJointX,
        this.config.rightArmJointY,
        rightArmAngle
      );
      const headTilt = (this.platformVel?.x || 0) * 8e-4;
      ctx.save();
    ...
    draw() {
      ...
      if (this.isDying) {
        ...
        const drawPos2 = this.isTeleporting ? this.teleportRenderPos : this.pos;
        ctx.translate(drawPos2.x, drawPos2.y);
  ...
  var AlienMonster = class {
    ...
    draw() {
      ...
      const pupilSizeSide = this.radius * 0.1;
